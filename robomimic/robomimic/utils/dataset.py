...
class SequenceDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        load_next_obs=True,
        interpolate=False,
        interpolate_horizon=None,
        interpolate_mode='linear',
        mapping_path=None,
        num_demo=100,
        num_robot_demo_for_mixup=5,
        dataset_human=False,
        action_stats_json_path=None,
    ):
        """
        ...
        Args:
            ...
            mapping (dict): only for human data, include {"human_demo_id": [["human_index_in_demo": ["robo_demo_id", "robo_index_in_demo"]], ...]}
        """
        ...
        if action_stats_json_path is not None:
            with open(action_stats_json_path, 'r') as f:
                self.action_normalization_stats = json.load(f, object_pairs_hook=OrderedDict)
                self.action_normalization_stats["action_absolute"]["scale"] = np.array(self.action_normalization_stats["action_absolute"]["scale"])
                self.action_normalization_stats["action_absolute"]["offset"] = np.array(self.action_normalization_stats["action_absolute"]["offset"])
        else:
            self.action_normalization_stats = None
        ...
        self.pair = True
        self.num_robot_demo_for_mixup = None # this is for the human dataset when select the pair dataset notice the situation
        if dataset_human:
            self.num_robot_demo_for_mixup = num_robot_demo_for_mixup
        else:
            self.pair = False
        ...
        self.interpolate_horizon = interpolate_horizon
        self.interpolate_mode = "nearest" # interpolate_mode
        if self.interpolate:
            assert self.hdf5_cache_mode in ["all", "low_dim"], "Interpolation is only supported in cache mode."
            assert self.interpolate_horizon is not None
    ...
    def load_demo_info(self, filter_by_attribute=None, demos=None):
        ...
        self.demos = sorted(self.demos, key=lambda x: int(x.split('_')[1]))

        self.demos = [demo for demo in self.demos if int(demo.split('_')[1]) < self.num_demo]
    ...
    def load_dataset_in_memory(self, demo_list, hdf5_file, obs_keys, dataset_keys, load_next_obs):
        ...
        for ep in LogUtils.custom_tqdm(demo_list):
            ...
            all_data[ep]["obs"] = {}
            for k in obs_keys:
                dataset_path = "data/{}/obs/{}".format(ep, k)
                # Check if the dataset exists in the file.
                if k == "wrist_image" and dataset_path not in hdf5_file:
                    all_data[ep]["obs"][k] = None
                else:
                    all_data[ep]["obs"][k] = hdf5_file[dataset_path][()]
            if load_next_obs:
    ...
    def get_action_stats(self):
        if self.action_stats is not None:
            print("SequenceDataset: using loaded action stats...")
    ...
    def get_dataset_for_ep(self, ep, key):
        ...
        if key_should_be_in_memory:
            ...
            if "wrist_image" in key and hd5key not in self.hdf5_file:
                ret = None
            else:
    ...
    def get_item(self, index):
        ...
        if not self.interpolate:
            ...
            raw_ac_dict = OrderedDict()
            for k in self.action_keys:
                raw_seq, _ = self.get_sequence_from_demo(
                    demo_id,
                    index_in_demo=index_in_demo,
                    keys=(k,),
                    num_frames_to_stack=self.n_frame_stack - 1,
                    seq_length=self.interpolate_horizon,
                )
                raw_ac_dict[k] = raw_seq[k]

            action_normalization_stats = self.get_action_normalization_stats()
            raw_ac_dict = ObsUtils.normalize_dict(raw_ac_dict, normalization_stats=action_normalization_stats)
            raw_actions = AcUtils.action_dict_to_vector(raw_ac_dict)

            target_length = self.seq_length
            indices = np.linspace(0, raw_actions.shape[0] - 1, target_length)
            if self.interpolate_mode == 'nearest':
                indices = np.round(indices).astype(int)
                meta["actions"] = raw_actions[indices]
            else:
                raise NotImplementedError

            raw_obs_dict = {}
            for k in self.obs_keys:
                # Use get_obs_sequence_from_demo with an interpolation horizon
                raw_obs = self.get_obs_sequence_from_demo(
                    demo_id,
                    index_in_demo=index_in_demo,
                    keys=(k,),
                    num_frames_to_stack=self.n_frame_stack - 1,
                ...
                if k == "wrist_image" and k not in raw_obs:
                    continue
                raw_obs_dict[k] = raw_obs[k]
                
            if self.interpolate_mode == 'nearest':
                for k in raw_obs_dict:
                    raw_obs_dict[k] = raw_obs_dict[k][indices]
            elif self.interpolate_mode == 'linear':
                raise NotImplementedError
                ...
                #     upper = int(np.ceil(x))
                #     if lower == upper:
                #         # x is an integer index; no interpolation needed
                #         interpolated_actions.append(raw_actions[lower])
                #     else:
                #         # Compute interpolation weights
                #         weight_upper = x - lower
                #         weight_lower = 1 - weight_upper
                #         interp_action = weight_lower * raw_actions[lower] + weight_upper * raw_actions[upper]
                #         interpolated_actions.append(interp_action)
                # meta["actions"] = np.stack(interpolated_actions, axis=0)
            else:
                raise NotImplementedError
            meta["obs"] = raw_obs_dict
        ...
        if self.pair and self.mapping:
            demo_id, index_in_demo = self.get_demo_id(index)
            meta["pair_index"] = list(random.choice([
                (int(k), v)
                for k, v in self.mapping[demo_id.removeprefix("demo_")][str(index_in_demo)].items()
                if int(k) < self.num_robot_demo_for_mixup
            ]))
    ...
    def get_sequence_from_demo(self, demo_id, index_in_demo, keys, num_frames_to_stack=0, seq_length=1):
        ...
        for k in keys:
            ...
            if "wrist_image" in k and data is None:
                continue
            else:
    ...
    def get_batch(self, index_list):
        robot_demo_id = index_list[0]
        robot_index_in_demo = index_list[1]
        robot_index = []
        for r_demo_id, r_ind_in_demo in zip(robot_demo_id, robot_index_in_demo):
            robot_index.append(self.get_index("demo_"+str(int(r_demo_id)), int(r_ind_in_demo)))

        batch = [self[i] for i in robot_index]
        return default_collate(batch)

    def get_demo_id(self, index):
        # Retrieve the demo id corresponding to the global index.
        demo_id = self._index_to_demo_id[index]
        # Get the start index for this demo.
        demo_start_index = self._demo_id_to_start_indices[demo_id]
        # Compute the local time index within the demo.
        # Note: if padding for frame stacking is disabled, we adjust by the offset.
        index_in_demo = index - demo_start_index
        if not self.pad_frame_stack:
            index_in_demo += self.n_frame_stack - 1
        return demo_id, index_in_demo

    def get_index(self, demo_id, index_in_demo):
        """
        Given a demo_id and a local time index (index_in_demo) within that demo,
        return the corresponding global dataset index.

        Args:
            demo_id (str): The demo identifier (e.g., "demo_0").
            index_in_demo (int): The local time index within the demo. This index is assumed
                                to be in the same "frame stack" space as returned by get_demo_id.

        Returns:
            int: The global dataset index corresponding to the specified demo and local index.
        """
        start_index = self._demo_id_to_start_indices[demo_id]
        if self.pad_frame_stack:
            global_index = start_index + index_in_demo
        else:
            # When frame stacking is not padded, get_demo_id adds an offset of (n_frame_stack - 1)
            # to the local index. We subtract that offset here to recover the original global index.
            global_index = start_index + index_in_demo - (self.n_frame_stack - 1)
        return global_index
...
def _aggregate_traj_stats(traj_stats_a, traj_stats_b):
    ...
    # print(f"merged_stats for actions: {merged_stats}")
    return merged_stats
...
def action_stats_to_normalization_stats(action_stats, action_config):
    ...
    for action_key in action_stats.keys():
        ...
        if norm_method is None:
            ...
            #     if dim not in dims_to_normalize:
            #         scale[0, dim]  = 1.0
            #         offset[0, dim] = 0.0
