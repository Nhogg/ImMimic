...
class DiffusionPolicyUNet(PolicyAlgo):
    ...
    def train_on_batch(self, batch, epoch, pair_batch=None, validate=False):
        ...
        with TorchUtils.maybe_no_grad(no_grad=validate):
            ...
            obs_features = TensorUtils.time_distributed(inputs, self.nets['policy']['obs_encoder'], inputs_as_kwargs=True) # [B, T, D]

            if pair_batch is not None:
                pair_inputs = {
                    'obs': pair_batch["obs"],
                    'goal': pair_batch.get("goal_obs", None)
                }
                pair_obs_features = TensorUtils.time_distributed(pair_inputs, self.nets['policy']['obs_encoder'], inputs_as_kwargs=True)
                merged_obs_features = self.alpha * obs_features + (1. - self.alpha) * pair_obs_features
                obs_cond = merged_obs_features.flatten(start_dim=1)
            else:
                obs_cond = obs_features.flatten(start_dim=1)
            
            if (pair_batch is not None) and (not validate):
                with torch.no_grad():
                    merged_actions = self.alpha * batch['actions'] + (1. - self.alpha) * pair_batch['actions']
                    batch['actions'] = merged_actions
    ...
    def get_action(self, obs_dict, goal_dict=None):
        ...
        # If the queue is full, remove the oldest observation and append the new one.
            self.obs_queue.popleft()
            self.obs_queue.append(obs_dict)
        
        
        # Convert list of dicts to a single dict with concatenated tensors
        obs_dict_concat = {}
        for k in obs_dict.keys():
            obs_dict_concat[k] = torch.cat([obs[k] for obs in self.obs_queue], dim=0)
        ...
        # run inference
        # [1,T,Da]
        action_sequence = self._get_action_trajectory(obs_dict=obs_dict_concat)[0]
    ...
    def _get_action_trajectory(self, obs_dict, goal_dict=None):
        ...
        for k in self.obs_shapes:
            if "image" in k:
                inputs['obs'][k] = inputs['obs'][k].permute(0, 3, 1, 2)
            inputs['obs'][k] = inputs['obs'][k].unsqueeze(0) # Batch_size
